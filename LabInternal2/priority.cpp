...
void display (process* q, int n ){
    ...
    for(int i=0;i<n;i++)
    {
        cout<<"P"<<q[i].pno<<"\t"<<q[i].arrival<<"\t"<<q[i].burst<<"\t"<<q[i].priority<<"\t\t"<<q[i].start<<"\t"<<q[i].finish<<"\t"<<q[i].wait<<"\t"<<q[i].turnaround<<endl;
        avgwait += q[i].wait;
...
process getMinPriority( process* q, int n, int arrival, int priority){
    int min = -1;
    for(int i = 0; i < n; i++){
        if(q[i].scheduled == 0 && q[i].arrival <= arrival && q[i].priority < priority ){
            min = i;
...
process* schedule (process* q, int n){
    ...
    for (int i = 0; i < n; i++){
        order[i] = getMinPriority(q, n, current, MAX_PRIOR);
...
int main () {
    ...
    cout << "Enter arrival, burst and priority of each process\n";
    for (int i = 0; i < n; i++){
        q[i].pno = i+1;
        q[i].scheduled = 0;
